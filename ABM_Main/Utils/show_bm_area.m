...
function [bm_area] = show_bm_area(BONE, site, bone_marrow_start_area, res_area_3_days, follow_up, show_plot, flag_resorption)
    
    if show_plot
        if flag_resorption
            % Compute normalized bone marrow area wrt its initial size
            for hour = 1 : follow_up 
                bm_area(hour) = (sum(sum(BONE(:, :, hour) == site.tumor_edge | BONE(:, :, hour) == site.tumor | BONE(:, :, hour) == site.vessel | BONE(:, :, hour) == site.vessel_cabo | BONE(:, :, hour) == site.bone_marrow)) - bone_marrow_start_area) / res_area_3_days;
            end
            clear hour
            ...
            figure
            hold on
            plot(3 * 24 : 1 : follow_up, bm_area(3 * 24 : follow_up), 'LineWidth', 1.5)
            % Set Labels
            xlabel('Time [h]')
