...
dovecercare = (vessels & ~bw2); % I search among points being not established vessels
[xxx,yyy] = find(dovecercare == 1);

if size(xxx,1)==0 %If the area is empty i go back to full tumor.
...
%     end 
    ...
    while (Flag_Vess_at_edge == 1)
          randomNumber = randi(size(xxx, 1)); %I randomly select the vessel that will do angiogenesis
          Starting_point = [ xxx_rand(randomNumber,1), yyy_rand(randomNumber,1)];
          Distance_vessels = (random(pd_Distance)/site_dim); % I draw from the distribution the distance from the mother vessel to the the borning vessel
          AngleVessel = deg2rad(random(pd_angles_vessels)); % I draw from the distribution the angle between the mother vessel and the borning vessel
          NewCenter = [Starting_point(1,1)-ceil(sin(AngleVessel)*Distance_vessels), Starting_point(1,2)+ceil(cos(AngleVessel)*Distance_vessels)];
...
%           end
          ...
          if bone(min(rows, Cyy+ceil(b_vessel/site_dim)),Cxx)>=site.tumor && bone(max(1,Cyy-ceil(b_vessel/site_dim)),Cxx)>=site.tumor && bone(Cyy,min(rows,Cxx+ceil(a_vessel/site_dim)))>=site.tumor && bone(Cyy,max(1,Cxx-ceil(a_vessel/site_dim)))>=site.tumor
             Flag_Vess_at_edge=0;
          end

          if  Flag_Vess_at_edge==1
              clear  NewCenter
          end 
    end 
...
%I update the bone matrix with new vessels
...
clear NewVesselCenter xxxxx  yyyyy bw xxx_rand  yyy_rand iii xxx  yyy  dovecercare