...
% curr_tumor_area = sum(sum(bone == site.tumor_edge | bone == site.tumor | bone == site.vessel | bone == site.vessel_cabo)) * site_dim * site_dim; %Area in squared Âµm           
...
dovecercare = (bone == site.tumor & ~bw2); % sites that could be adressed for a new vessel (bw2 è la maschera gialla quadrata che nasce dalla funzione creomaskvasi che fa in modo che non creascano nuovi vasi nella zona centrale del tumore che già ha una densità maggiore)
